...
if (isset($_POST['submit'])) {
	...
	$query = <<<SQL
		select 
			...
			total, 
			card_no, 
			memb.FirstName AS first, 
			memb.LastName AS last, 
			memb.addshopper, 
			memb.sponsorCardNo, 
			spons.FirstName as sponsfirst, 
			spons.LastName as sponslast 
		from dtransactions 
		LEFT JOIN is4c_op.members memb ON dtransactions.card_no = memb.CardNo 
			AND memb.id = 
				(select id 
				from is4c_op.members 
				where CardNo = dtransactions.card_no limit 1) 
		LEFT JOIN is4c_op.members spons ON memb.sponsorCardNo = spons.CardNo 
			AND spons.id = 
				(select id 
				from is4c_op.members 
				where CardNo = memb.sponsorCardNo limit 1) 
		WHERE $ourwhere 
...
error_log("in trans loop card no: " . $trans['cardno']);
		$member_totals[$trans['cardno']] += $trans['amount'];
		$member_data[$trans['cardno']] = array(
			"firstname" => $trans['firstname'],
			"lastname" => $trans['lastname'],
			'sponsorcard' => $trans['sponsorcard'],
			'sponsfirst' => $trans['sponsfirst'],
			'sponslast' => $trans['sponslast'],
			'addshopper' => $trans['addshopper']
		);

	}

error_log("member_data: " . var_export($member_data, true));
	...
	$html .= "<h3>Member Totals</h3>";

	$html .= "<table>";
	$html .= '<tr><th align="left">Card Num</th> <th align="left">First</th> <th align="left">Last</th> <th align="left">Member Type</th><th align="left">Total</th> </tr>';

	foreach ($member_data as $cardno => $meminfo) {
		if (!$csvexport)
			$html .= "<tr>" .
			"<td>" . $cardno . "</td>" .
			"<td>" . $meminfo['firstname'] . "</td>" .
			"<td>" . $meminfo['lastname'] . "</td>" .
			"<td>" . ( $meminfo['addshopper'] ? ("Sponsored (" . $meminfo['sponsorcard'] . ':' . $meminfo['sponsfirst'] . ' ' . $meminfo['sponslast'] . ")") : ("Full") ) . "</td>" .
			"<td align=\"right\">" . cashformat($member_totals[$cardno]) . "</td>" .
			"</tr>\n";
		else
			$csvdata[] = array_merge(
				array("cardno" => $cardno),
				$meminfo,
				array("total" => $member_totals[$cardno])
			);
	}
	$html .= "</table>";
	...
	$html .= startform();
	$html .= "<h2>Member Purchases Totals</h2>";
