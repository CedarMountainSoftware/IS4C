...
//FILE *fp_scannertest;
//fp_scannertest = fopen("/pos/is4c/rs232/ssddata/scannertest", "w");

				serialBuffer[num] = '\0';

				/**************** process scanned data ****************/
//this was put in cause the scale is not zeroing. TAKE THIS OUT WITH A FIXED/NEW SCALE
if((serialBuffer[0] == 'S')&&(serialBuffer[1] == '1')&&(serialBuffer[2] == '4')&&(serialBuffer[3] == '1'))
{
}else
				if ((serialBuffer[1] == '0')||(newway==1)) 
				{
					if(newway==1)
					{
//fprintf(fp_scannertest, "serial buf ---- %s\n\n", serialBuffer);
//for (i=1; i<18; i++) scannerInput[i]='\0';
						
						for (i=1; i<14; i++) 
						{
							scannerInput[i-1] = serialBuffer[i];
//fprintf(fp_scannertest, "scannerInput buf ---- %s\n", scannerInput);
						}
						scannerInput[13]='\0';
					}
					else
